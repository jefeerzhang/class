...
print("分隔符行:", separator_lines)
print("分隔符数量:", len(separator_lines))
...
for r in results:
    ...
    print(str(r['page']) + ' ' + str(r['lines']) + ' ' + str(r['max_len']) + ' ' + str(r['list_items']) + ' ' + r['density'] + ' ' + status)
...
for page_num, page_lines in enumerate(pages, 1):
    ...
    for j, para in enumerate(paragraphs):
        para_lines = para.split('\n')
        if len(para_lines) > 3:
            print('第' + str(page_num) + '页，段落' + str(j+1) + '：' + str(len(para_lines)) + '行')
