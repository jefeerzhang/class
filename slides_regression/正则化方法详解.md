...
$$
L = \sum_{i=1}^{n} (y_i - \hat{y}_i)^2 + \lambda \left[ \alpha \sum_{j=1}^{p} |\beta_j| + (1-\alpha) \sum_{j=1}^{p} \beta_j^2 \right]
$$
...
|------|-----|-----------|-----------|------------|
| 惩罚项 | 无 | $\sum \beta_j^2$ | $\sum \lvert\beta_j\rvert$ | $\alpha \sum \lvert\beta_j\rvert + (1-\alpha) \sum \beta_j^2$ |
| 稀疏性 | 否 | 否 | 是 | 是 |
| 特征选择 | 无 | 无 | 自动 | 自动 |
| 共线性处理 | 差 | 好 | 一般 | 好 |
