...
2. **修正久期（Modified Duration）**：利率变化 1 个百分点时，债券价格变化的百分比。例如修正久期 = 2.7，意味着利率每升 1%，价格约跌 2.7%

前者是"时间"概念，后者是"敏感度"概念。我们从前者开始。

> **类比：久期 = 利率的"力臂"**。就像撬棍——力臂越长，同样的力能撬动更大的重量。一张 30 年期零息债券的久期 = 30 年，相当于 1 年期债券的 30 倍力臂，利率稍微动一下，长债的价格就被撬得飞起。
...
- 零息债券：只有 `t = n` 一笔现金流，权重 = 1，所以 `D_Mac = n`
- 附息债券：前面有若干利息现金流，权重分散了，所以 `D_Mac < n`
...
| 期数 (t) | 现金流 | 折现因子 `1 / (1.06)^t` | 现值 | `t ×` 现值 |
|:---------:|:------:|:--------------------:|:----:|:--------------:|
...
上面的例子中：
- 美元久期 = `2.70 × 97.33 = 262.79`
- 利率上升 1%（即 0.01）：价格变化 `≈ -262.79 × 0.01 = -2.63` 元
...
但需要注意：**久期随期限增长的速度是递减的**——这就是 Malkiel 定理四的含义：从 1 年期变 2 年期，久期增加近 1 年；但从 20 年期变 21 年期，久期只增加零点几年。
...
| 零息债券 | 久期 = 期限 | 仅一次现金流 |
| 永续债券 | 久期 = `(1+r)/r` | 无穷级数求和；例：r=5% → D=21 年，对利率极度敏感 |
...
def macaulay_duration(face_value, coupon_rate, ytm, years, freq=1):
    ...
    # t 以"期数"为单位（半年付息时 t=1,2,...,2n）
    # 除以 price 得到期数单位的久期，再除以 freq 转为"年"单位
...
**为什么刚好抵消？** 当组合久期 = 负债期限时，价格风险与再投资风险恰好等大反向：久期越长，价格对利率越敏感（跌得多），但利息再投资的期限也越长（赚得多）——两条效应在久期=期限这个点上相互抵消，就像一架**天平**，两边重量相等，利率的风吹过来也不会倾斜。
...
**现实案例**：2023 年美国硅谷银行（SVB）危机的核心就是久期错配——

1. SVB 吸收了大量短期存款（负债久期极短，几乎为零）
2. 将资金大量配置于长期国债和 MBS（资产久期很长，约 5-6 年）
3. 美联储快速加息 → 长期国债市价暴跌 → 按市值计价出现巨额浮亏
4. 消息传出 → 储户集中挤兑 → SVB 被迫抛售债券变现
5. 浮亏变实亏 → 资不抵债 → 48 小时内倒闭

整个过程的关键推手就是**正久期缺口**：资产久期远大于负债久期，加息时资产跌得比负债多，净资产迅速蒸发。
...
> **公式来源**：上式是 `d²P/dr² ÷ P`（价格对收益率的二阶导数除以价格）。`t(t+1)/(1+r)^(t+2)` 来自对折现因子 `1/(1+r)^t` 求两次导：第一次得 `-t/(1+r)^(t+1)`，第二次得 `t(t+1)/(1+r)^(t+2)`。
...
**类比一：斜率 vs 弯曲度**
- 久期 = **曲线的斜率**（一阶效应）——告诉你当前点的瞬时变化率
- 凸性 = **曲线的弯曲度**（二阶效应）——告诉你斜率本身在如何变化
- 只看斜率，只能沿切线做线性外推
- 加上弯曲度，才能追踪曲线的真实走向

**类比二：车速 vs 油门**
- 久期 = 你当前的**车速**——知道车速，只能预测匀速前进的终点
- 凸性 = **油门/刹车的深浅**——知道油门深浅，才能判断接下来是加速还是减速

**类比三：凸性 = 弹簧**
凸性就像债券自带的**弹簧**——利率推你的时候弹簧帮你缓冲（少跌一点），利率拉你的时候弹簧帮你回弹（多涨一点）。弹簧越硬（凸性越大），这种"涨多跌少"的效应越明显。
...
右图更清楚地展示了这个差异（误差）。曲线在 `Δr = 0` 处为 0，向两边延伸时始终为正。

**结论**：凸性对投资者有利！它让债券在利率下降时多涨，在利率上升时少跌。

但注意：高凸性的债券通常收益率较低——"天下没有免费的午餐"，投资者为凸性支付了溢价（就像为弹簧付出额外成本）。
