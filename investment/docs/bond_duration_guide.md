...
> **💡 读前提示**：以下推导涉及微积分。不熟悉求导的读者可以直接跳到本小节末尾的核心公式（方框处），不影响理解后续内容。
...
> **公式来源**：上式是 $\frac{d^2P}{dr^2} / P$（价格对收益率的二阶导数除以价格）。$t(t+1)/(1+r)^{t+2}$ 来自对折现因子 $1/(1+r)^t$ 求两次导：第一次得 $-t/(1+r)^{t+1}$，第二次得 $t(t+1)/(1+r)^{t+2}$。
